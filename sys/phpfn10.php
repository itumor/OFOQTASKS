...
function CurrentProjectID() {
	...
	return "{3246B9FA-4C51-4733-8040-34B188FCD87E}";
}
...
class cAdvancedSecurity {
	...
	function ValidateUser(&$usr, &$pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			$sSql = $_login->GetSQL($sFilter, "");
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ew_ComparePassword($rs->fields('loginpassword'), $pwd);
					if ($ValidateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
						$_SESSION[EW_SESSION_SYS_ADMIN] = 0; // Non System Administrator
						$this->setCurrentUserName($rs->fields('loginname')); // Load user name
						$this->setSessionUserID($rs->fields('idlogin')); // Load User ID
						if (is_null($rs->fields('levels'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('levels'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sFilter = $_login->UserIDFilter($this->CurrentUserID);
			$sSql = $_login->GetSQL($sFilter, '');
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('idlogin'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
class cUserProfile {
	...
	function SaveProfileToDatabase($usr) {
		...
		if ($usr == "" || $usr == EW_ADMIN_USER_NAME) // Ignore hard code admin
			return FALSE;
		$sFilter = str_replace("%u", ew_AdjustSql($usr), EW_USER_NAME_FILTER);
		$sSql = "UPDATE " . ew_QuotedName($_login->TableName) .
