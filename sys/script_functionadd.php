...
$script_function_add = NULL; // Initialize page object first

class cscript_function_add extends cscript_function {
	...
	// Page object name
	var $PageObjName = 'script_function_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["script_function_id"] != "") {
				...
				$this->setKey("script_function_id", $this->script_function_id->CurrentValue); // Set up key
			} else {
				$this->setKey("script_function_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("script_functionlist.php"); // No matching record, return to list
				}
				break;
	...
	function LoadDefaultValues() {
		$this->script_function_name->CurrentValue = NULL;
		$this->script_function_name->OldValue = $this->script_function_name->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// script_function_name
		$this->script_function_name->SetDbValueDef($rsnew, $this->script_function_name->CurrentValue, "", FALSE);
		...
		if ($AddRow) {
			$this->script_function_id->setDbValue($conn->Insert_ID());
			$rsnew['script_function_id'] = $this->script_function_id->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($script_function_add)) $script_function_add = new cscript_function_add();

// Page init
$script_function_add->Page_Init();

// Page main
$script_function_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$script_function_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var script_function_add = new ew_Page("script_function_add");
script_function_add.PageID = "add"; // Page ID
var EW_PAGE_ID = script_function_add.PageID; // For backward compatibility

// Form object
var fscript_functionadd = new ew_Form("fscript_functionadd");

// Validate form
fscript_functionadd.Validate = function() {
...
fscript_functionadd.Form_CustomValidate = 
...
fscript_functionadd.ValidateRequired = true;
<?php } else { ?>
fscript_functionadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fscript_functionadd.Lists["x_script_id"] = {"LinkField":"x_script_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_script_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $script_function_add->ShowPageHeader(); ?>
<?php
$script_function_add->ShowMessage();
?>
<form name="fscript_functionadd" id="fscript_functionadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<table id="tbl_script_functionadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fscript_functionadd.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$script_function_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$script_function_add->Page_Terminate();
?>
