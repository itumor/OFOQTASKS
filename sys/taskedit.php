...
$task_edit = NULL; // Initialize page object first

class ctask_edit extends ctask {
...
// Create page object
if (!isset($task_edit)) $task_edit = new ctask_edit();
...
$task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var task_edit = new ew_Page("task_edit");
task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = task_edit.PageID; // For backward compatibility

// Form object
var ftaskedit = new ew_Form("ftaskedit");

// Validate form
ftaskedit.Validate = function() {
...
ftaskedit.ValidateRequired = true;
<?php } else { ?>
ftaskedit.ValidateRequired = false; 
<?php } ?>
...
<?php $task_edit->ShowPageHeader(); ?>
<?php
$task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($task_edit->Pager)) $task_edit->Pager = new cNumericPager($task_edit->StartRec, $task_edit->DisplayRecs, $task_edit->TotalRecs, $task_edit->RecRange) ?>
<?php if ($task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $task_edit->PageUrl() ?>start=<?php echo $task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $task_edit->PageUrl() ?>start=<?php echo $task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $task_edit->PageUrl() ?>start=<?php echo $task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $task_edit->PageUrl() ?>start=<?php echo $task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="ftaskedit" id="ftaskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<table id="tbl_taskedit" class="table table-bordered table-striped">
...
<input type="hidden" data-field="x_task_id" name="x_task_id" id="x_task_id" value="<?php echo ew_HtmlEncode($task->task_id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_task_task_id" class="control-group">
<span<?php echo $task->task_id->ViewAttributes() ?>>
<?php echo $task->task_id->ViewValue ?></span>
</span>
<input type="hidden" data-field="x_task_id" name="x_task_id" id="x_task_id" value="<?php echo ew_HtmlEncode($task->task_id->FormValue) ?>">
<?php } ?>
<?php echo $task->task_id->CustomMsg ?></td>
	</tr>
<?php } ?>
