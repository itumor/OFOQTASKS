...
$create_task_delete = NULL; // Initialize page object first

class ccreate_task_delete extends ccreate_task {
	...
	// Page object name
	var $PageObjName = 'create_task_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("create_tasklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in create_task class, create_taskinfo.php
...
// Create page object
if (!isset($create_task_delete)) $create_task_delete = new ccreate_task_delete();

// Page init
$create_task_delete->Page_Init();

// Page main
$create_task_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$create_task_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var create_task_delete = new ew_Page("create_task_delete");
create_task_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = create_task_delete.PageID; // For backward compatibility

// Form object
var fcreate_taskdelete = new ew_Form("fcreate_taskdelete");

// Form_CustomValidate event
fcreate_taskdelete.Form_CustomValidate = 
...
fcreate_taskdelete.ValidateRequired = true;
<?php } else { ?>
fcreate_taskdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcreate_taskdelete.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcreate_taskdelete.Lists["x_HOSTNAME"] = {"LinkField":"x_server_hostname","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($create_task_delete->Recordset = $create_task_delete->LoadRecordset())
	$create_task_deleteTotalRecs = $create_task_delete->Recordset->RecordCount(); // Get record count
if ($create_task_deleteTotalRecs <= 0) { // No record found, exit
	if ($create_task_delete->Recordset)
		$create_task_delete->Recordset->Close();
	$create_task_delete->Page_Terminate("create_tasklist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $create_task_delete->ShowPageHeader(); ?>
<?php
$create_task_delete->ShowMessage();
?>
<form name="fcreate_taskdelete" id="fcreate_taskdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="create_task">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($create_task_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_create_taskdelete" class="ewTable ewTableSeparate">
<?php echo $create_task->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($create_task->id->Visible) { // id ?>
		<td><span id="elh_create_task_id" class="create_task_id"><?php echo $create_task->id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($create_task->server_id_mysqladmin->Visible) { // server_id_mysqladmin ?>
		<td><span id="elh_create_task_server_id_mysqladmin" class="create_task_server_id_mysqladmin"><?php echo $create_task->server_id_mysqladmin->FldCaption() ?></span></td>
<?php } ?>
...
<?php } ?>
	...
	<tbody>
<?php
$create_task_delete->RecCnt = 0;
$i = 0;
while (!$create_task_delete->Recordset->EOF) {
	$create_task_delete->RecCnt++;
	$create_task_delete->RowCnt++;

	// Set row properties
	$create_task->ResetAttrs();
	$create_task->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$create_task_delete->LoadRowValues($create_task_delete->Recordset);

	// Render row
	$create_task_delete->RenderRow();
?>
	<tr<?php echo $create_task->RowAttributes() ?>>
<?php if ($create_task->id->Visible) { // id ?>
		<td<?php echo $create_task->id->CellAttributes() ?>>
<span id="el<?php echo $create_task_delete->RowCnt ?>_create_task_id" class="control-group create_task_id">
...
<span id="el<?php echo $create_task_delete->RowCnt ?>_create_task_server_id_mysqladmin" class="control-group create_task_server_id_mysqladmin">
...
<span id="el<?php echo $create_task_delete->RowCnt ?>_create_task_HOSTNAME" class="control-group create_task_HOSTNAME">
...
<span id="el<?php echo $create_task_delete->RowCnt ?>_create_task_DATABASE" class="control-group create_task_DATABASE">
...
<?php
	$create_task_delete->Recordset->MoveNext();
}
$create_task_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
fcreate_taskdelete.Init();
</script>
<?php
$create_task_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$create_task_delete->Page_Terminate();
?>
