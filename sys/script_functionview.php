...
$script_function_view = NULL; // Initialize page object first

class cscript_function_view extends cscript_function {
	...
	// Page object name
	var $PageObjName = 'script_function_view';
	...
	function __construct() {
		...
		if (@$_GET["script_function_id"] <> "") {
			$this->RecKey["script_function_id"] = $_GET["script_function_id"];
			$KeyUrl .= "&script_function_id=" . urlencode($this->RecKey["script_function_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["script_function_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["script_function_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "script_functionlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "script_functionlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_script_function\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_script_function',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fscript_functionview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($script_function_view)) $script_function_view = new cscript_function_view();

// Page init
$script_function_view->Page_Init();

// Page main
$script_function_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$script_function_view->Page_Render();
?>
...
// Page object
var script_function_view = new ew_Page("script_function_view");
script_function_view.PageID = "view"; // Page ID
var EW_PAGE_ID = script_function_view.PageID; // For backward compatibility

// Form object
var fscript_functionview = new ew_Form("fscript_functionview");

// Form_CustomValidate event
fscript_functionview.Form_CustomValidate = 
...
fscript_functionview.ValidateRequired = true;
<?php } else { ?>
fscript_functionview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fscript_functionview.Lists["x_script_id"] = {"LinkField":"x_script_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_script_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $script_function_view->ExportOptions->Render("body") ?>
<?php if (!$script_function_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($script_function_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $script_function_view->ShowPageHeader(); ?>
<?php
$script_function_view->ShowMessage();
?>
<?php if ($script_function->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($script_function_view->Pager)) $script_function_view->Pager = new cNumericPager($script_function_view->StartRec, $script_function_view->DisplayRecs, $script_function_view->TotalRecs, $script_function_view->RecRange) ?>
<?php if ($script_function_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($script_function_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $script_function_view->PageUrl() ?>start=<?php echo $script_function_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($script_function_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $script_function_view->PageUrl() ?>start=<?php echo $script_function_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($script_function_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $script_function_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($script_function_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $script_function_view->PageUrl() ?>start=<?php echo $script_function_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($script_function_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $script_function_view->PageUrl() ?>start=<?php echo $script_function_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="fscript_functionview" id="fscript_functionview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="script_function">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_script_functionview" class="table table-bordered table-striped">
...
<script type="text/javascript">
fscript_functionview.Init();
</script>
<?php
$script_function_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($script_function->Export == "") { ?>
...
<?php
$script_function_view->Page_Terminate();
?>
