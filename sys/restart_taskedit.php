...
$restart_task_edit = NULL; // Initialize page object first

class crestart_task_edit extends crestart_task {
	...
	// Page object name
	var $PageObjName = 'restart_task_edit';
...
// Create page object
if (!isset($restart_task_edit)) $restart_task_edit = new crestart_task_edit();

// Page init
$restart_task_edit->Page_Init();

// Page main
$restart_task_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$restart_task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var restart_task_edit = new ew_Page("restart_task_edit");
restart_task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = restart_task_edit.PageID; // For backward compatibility

// Form object
var frestart_taskedit = new ew_Form("frestart_taskedit");

// Validate form
frestart_taskedit.Validate = function() {
...
frestart_taskedit.Form_CustomValidate = 
...
frestart_taskedit.ValidateRequired = true;
<?php } else { ?>
frestart_taskedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
frestart_taskedit.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $restart_task_edit->ShowPageHeader(); ?>
<?php
$restart_task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($restart_task_edit->Pager)) $restart_task_edit->Pager = new cNumericPager($restart_task_edit->StartRec, $restart_task_edit->DisplayRecs, $restart_task_edit->TotalRecs, $restart_task_edit->RecRange) ?>
<?php if ($restart_task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($restart_task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $restart_task_edit->PageUrl() ?>start=<?php echo $restart_task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($restart_task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $restart_task_edit->PageUrl() ?>start=<?php echo $restart_task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($restart_task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $restart_task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($restart_task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $restart_task_edit->PageUrl() ?>start=<?php echo $restart_task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($restart_task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $restart_task_edit->PageUrl() ?>start=<?php echo $restart_task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="frestart_taskedit" id="frestart_taskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="restart_task">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_restart_taskedit" class="table table-bordered table-striped">
...
<?php echo $restart_task->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($restart_task->id->CurrentValue) ?>">
<?php echo $restart_task->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
frestart_taskedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$restart_task_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$restart_task_edit->Page_Terminate();
?>
