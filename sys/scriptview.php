...
$script_view = NULL; // Initialize page object first

class cscript_view extends cscript {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["script_id"] <> "") {
			$this->RecKey["script_id"] = $_GET["script_id"];
			$KeyUrl .= "&script_id=" . urlencode($this->RecKey["script_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["script_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["script_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "scriptlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "scriptlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_script\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_script',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fscriptview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($script_view)) $script_view = new cscript_view();

// Page init
$script_view->Page_Init();

// Page main
$script_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$script_view->Page_Render();
?>
...
// Page object
var script_view = new ew_Page("script_view");
script_view.PageID = "view"; // Page ID
var EW_PAGE_ID = script_view.PageID; // For backward compatibility

// Form object
var fscriptview = new ew_Form("fscriptview");

// Form_CustomValidate event
fscriptview.Form_CustomValidate = 
...
fscriptview.ValidateRequired = true;
<?php } else { ?>
fscriptview.ValidateRequired = false; 
<?php } ?>
...
<?php $script_view->ExportOptions->Render("body") ?>
<?php if (!$script_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($script_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $script_view->ShowPageHeader(); ?>
<?php
$script_view->ShowMessage();
?>
<?php if ($script->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($script_view->Pager)) $script_view->Pager = new cNumericPager($script_view->StartRec, $script_view->DisplayRecs, $script_view->TotalRecs, $script_view->RecRange) ?>
<?php if ($script_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($script_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $script_view->PageUrl() ?>start=<?php echo $script_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($script_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $script_view->PageUrl() ?>start=<?php echo $script_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($script_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $script_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($script_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $script_view->PageUrl() ?>start=<?php echo $script_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($script_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $script_view->PageUrl() ?>start=<?php echo $script_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="fscriptview" id="fscriptview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="script">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_scriptview" class="table table-bordered table-striped">
...
<?php echo $script->script_path->ViewValue ?>
