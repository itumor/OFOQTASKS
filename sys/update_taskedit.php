...
$update_task_edit = NULL; // Initialize page object first

class cupdate_task_edit extends cupdate_task {
	...
	// Page object name
	var $PageObjName = 'update_task_edit';
...
// Create page object
if (!isset($update_task_edit)) $update_task_edit = new cupdate_task_edit();

// Page init
$update_task_edit->Page_Init();

// Page main
$update_task_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$update_task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var update_task_edit = new ew_Page("update_task_edit");
update_task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = update_task_edit.PageID; // For backward compatibility

// Form object
var fupdate_taskedit = new ew_Form("fupdate_taskedit");

// Validate form
fupdate_taskedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (felm && elm && !ew_HasValue(elm))
				return this.OnError(felm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($update_task->FILEPATH->FldCaption()) ?>");
			felm = this.GetElements("x" + infix + "_FILENAME");
			elm = this.GetElements("fn_x" + infix + "_FILENAME");
			if (felm && elm && !ew_HasValue(elm))
				return this.OnError(felm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($update_task->FILENAME->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_DBUSERNAME");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($update_task->DBUSERNAME->FldCaption()) ?>");
...
fupdate_taskedit.Form_CustomValidate = 
...
fupdate_taskedit.ValidateRequired = true;
<?php } else { ?>
fupdate_taskedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fupdate_taskedit.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fupdate_taskedit.Lists["x_HOSTNAME"] = {"LinkField":"x_server_hostname","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $update_task_edit->ShowPageHeader(); ?>
<?php
$update_task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($update_task_edit->Pager)) $update_task_edit->Pager = new cNumericPager($update_task_edit->StartRec, $update_task_edit->DisplayRecs, $update_task_edit->TotalRecs, $update_task_edit->RecRange) ?>
<?php if ($update_task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($update_task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $update_task_edit->PageUrl() ?>start=<?php echo $update_task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($update_task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $update_task_edit->PageUrl() ?>start=<?php echo $update_task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($update_task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $update_task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($update_task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $update_task_edit->PageUrl() ?>start=<?php echo $update_task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($update_task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $update_task_edit->PageUrl() ?>start=<?php echo $update_task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="fupdate_taskedit" id="fupdate_taskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="update_task">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_update_taskedit" class="table table-bordered table-striped">
...
<?php echo $update_task->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($update_task->id->CurrentValue) ?>">
<?php echo $update_task->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<select data-field="x_server_id_mysqladmin" id="x_server_id_mysqladmin" name="x_server_id_mysqladmin"<?php echo $update_task->server_id_mysqladmin->EditAttributes() ?>>
<?php
if (is_array($update_task->server_id_mysqladmin->EditValue)) {
	$arwrk = $update_task->server_id_mysqladmin->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($update_task->server_id_mysqladmin->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
// Call Lookup selecting
$update_task->Lookup_Selecting($update_task->server_id_mysqladmin, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<select data-field="x_HOSTNAME" id="x_HOSTNAME" name="x_HOSTNAME"<?php echo $update_task->HOSTNAME->EditAttributes() ?>>
<?php
if (is_array($update_task->HOSTNAME->EditValue)) {
	$arwrk = $update_task->HOSTNAME->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($update_task->HOSTNAME->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
// Call Lookup selecting
$update_task->Lookup_Selecting($update_task->HOSTNAME, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<input type="hidden" name="fn_x_FILEPATH" id= "fn_x_FILEPATH" value="<?php echo $update_task->FILEPATH->Upload->FileName ?>">
...
<input type="hidden" name="fn_x_FILENAME" id= "fn_x_FILENAME" value="<?php echo $update_task->FILENAME->Upload->FileName ?>">
...
<?php if ($update_task->DBUSERNAME->Visible) { // DBUSERNAME ?>
	<tr id="r_DBUSERNAME">
		<td><span id="elh_update_task_DBUSERNAME"><?php echo $update_task->DBUSERNAME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $update_task->DBUSERNAME->CellAttributes() ?>>
<span id="el_update_task_DBUSERNAME" class="control-group">
<input type="text" data-field="x_DBUSERNAME" name="x_DBUSERNAME" id="x_DBUSERNAME" size="30" maxlength="255" placeholder="<?php echo $update_task->DBUSERNAME->PlaceHolder ?>" value="<?php echo $update_task->DBUSERNAME->EditValue ?>"<?php echo $update_task->DBUSERNAME->EditAttributes() ?>>
</span>
<?php echo $update_task->DBUSERNAME->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
...
<script type="text/javascript">
fupdate_taskedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$update_task_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$update_task_edit->Page_Terminate();
?>
