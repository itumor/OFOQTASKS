...
//
// Table class for fabsprop_task
//
class cfabsprop_task extends cTable {
	var $id;
	var $username;
	var $datetime;
	...
	function __construct() {
		...
		$this->TableVar = 'fabsprop_task';
		$this->TableName = 'fabsprop_task';
		...
		// id
		$this->id = new cField('fabsprop_task', 'fabsprop_task', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// username
		$this->username = new cField('fabsprop_task', 'fabsprop_task', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['username'] = &$this->username;

		// datetime
		$this->datetime = new cField('fabsprop_task', 'fabsprop_task', 'x_datetime', 'datetime', '`datetime`', 'DATE_FORMAT(`datetime`, \'%d/%m/%Y %H:%i:%s\')', 135, -1, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['datetime'] = &$this->datetime;

		// server_id_glassfishscript
		$this->server_id_glassfishscript = new cField('fabsprop_task', 'fabsprop_task', 'x_server_id_glassfishscript', 'server_id_glassfishscript', '`server_id_glassfishscript`', '`server_id_glassfishscript`', 200, -1, FALSE, '`server_id_glassfishscript`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['server_id_glassfishscript'] = &$this->server_id_glassfishscript;

		// glassfishfolder
		$this->glassfishfolder = new cField('fabsprop_task', 'fabsprop_task', 'x_glassfishfolder', 'glassfishfolder', '`glassfishfolder`', '`glassfishfolder`', 200, -1, FALSE, '`glassfishfolder`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['glassfishfolder'] = &$this->glassfishfolder;

		// domainname
		$this->domainname = new cField('fabsprop_task', 'fabsprop_task', 'x_domainname', 'domainname', '`domainname`', '`domainname`', 200, -1, FALSE, '`domainname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['domainname'] = &$this->domainname;

		// contextname_value
		$this->contextname_value = new cField('fabsprop_task', 'fabsprop_task', 'x_contextname_value', 'contextname_value', '`contextname_value`', '`contextname_value`', 200, -1, FALSE, '`contextname_value`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['contextname_value'] = &$this->contextname_value;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("fabsprop_taskview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("fabsprop_taskview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "fabsprop_taskadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("fabsprop_taskedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("fabsprop_taskadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("fabsprop_taskdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->contextname_value->Exportable) $Doc->ExportCaption($this->contextname_value);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->username->Exportable) $Doc->ExportCaption($this->username);
				if ($this->datetime->Exportable) $Doc->ExportCaption($this->datetime);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->contextname_value->Exportable) $Doc->ExportField($this->contextname_value);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->username->Exportable) $Doc->ExportField($this->username);
					if ($this->datetime->Exportable) $Doc->ExportField($this->datetime);
