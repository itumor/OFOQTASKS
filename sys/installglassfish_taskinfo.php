...
$installglassfish_task = NULL;

//
// Table class for installglassfish_task
//
class cinstallglassfish_task extends cTable {
	var $id;
	var $username;
	var $datetime;
	...
	function __construct() {
		...
		$this->TableVar = 'installglassfish_task';
		$this->TableName = 'installglassfish_task';
		...
		// id
		$this->id = new cField('installglassfish_task', 'installglassfish_task', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// username
		$this->username = new cField('installglassfish_task', 'installglassfish_task', 'x_username', 'username', '`username`', '`username`', 200, -1, FALSE, '`username`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['username'] = &$this->username;

		// datetime
		$this->datetime = new cField('installglassfish_task', 'installglassfish_task', 'x_datetime', 'datetime', '`datetime`', 'DATE_FORMAT(`datetime`, \'%d/%m/%Y %H:%i:%s\')', 135, -1, FALSE, '`datetime`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['datetime'] = &$this->datetime;

		// server_id_glassfishscript
		$this->server_id_glassfishscript = new cField('installglassfish_task', 'installglassfish_task', 'x_server_id_glassfishscript', 'server_id_glassfishscript', '`server_id_glassfishscript`', '`server_id_glassfishscript`', 200, -1, FALSE, '`server_id_glassfishscript`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['server_id_glassfishscript'] = &$this->server_id_glassfishscript;

		// glassfishfolder
		$this->glassfishfolder = new cField('installglassfish_task', 'installglassfish_task', 'x_glassfishfolder', 'glassfishfolder', '`glassfishfolder`', '`glassfishfolder`', 200, -1, FALSE, '`glassfishfolder`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['glassfishfolder'] = &$this->glassfishfolder;

		// adminpassword
		$this->adminpassword = new cField('installglassfish_task', 'installglassfish_task', 'x_adminpassword', 'adminpassword', '`adminpassword`', '`adminpassword`', 200, -1, FALSE, '`adminpassword`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['adminpassword'] = &$this->adminpassword;
	}
	...
	// Update Table
	var $UpdateTable = "`installglassfish_task`";
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "installglassfish_tasklist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("installglassfish_taskview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("installglassfish_taskview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "installglassfish_taskadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("installglassfish_taskedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("installglassfish_taskadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("installglassfish_taskdelete.php", $this->UrlParm());
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				if ($this->adminpassword->Exportable) $Doc->ExportCaption($this->adminpassword);
			} else {
				if ($this->id->Exportable) $Doc->ExportCaption($this->id);
				if ($this->username->Exportable) $Doc->ExportCaption($this->username);
				if ($this->datetime->Exportable) $Doc->ExportCaption($this->datetime);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					if ($this->adminpassword->Exportable) $Doc->ExportField($this->adminpassword);
				} else {
					if ($this->id->Exportable) $Doc->ExportField($this->id);
					if ($this->username->Exportable) $Doc->ExportField($this->username);
					if ($this->datetime->Exportable) $Doc->ExportField($this->datetime);
