...
$restore_task_FILENAME_blobview = NULL; // Initialize page object first

class crestore_task_FILENAME_blobview extends crestore_task {
	...
	// Page object name
	var $PageObjName = 'restore_task_FILENAME_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'restore_task');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('restore_task', 'x_FILENAME');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('FILENAME');
				...
				if (trim(strval($this->Recordset->fields('FILENAME'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('FILENAME') . "\"");
				}
...
// Create page object
if (!isset($restore_task_FILENAME_blobview)) $restore_task_FILENAME_blobview = new crestore_task_FILENAME_blobview();

// Page init
$restore_task_FILENAME_blobview->Page_Init();

// Page main
$restore_task_FILENAME_blobview->Page_Main();
?>
<?php
$restore_task_FILENAME_blobview->Page_Terminate();
?>
