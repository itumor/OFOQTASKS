...
$drop_task_edit = NULL; // Initialize page object first

class cdrop_task_edit extends cdrop_task {
	...
	// Page object name
	var $PageObjName = 'drop_task_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("drop_tasklist.php"); // Return to list page
...
// Create page object
if (!isset($drop_task_edit)) $drop_task_edit = new cdrop_task_edit();

// Page init
$drop_task_edit->Page_Init();

// Page main
$drop_task_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$drop_task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var drop_task_edit = new ew_Page("drop_task_edit");
drop_task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = drop_task_edit.PageID; // For backward compatibility

// Form object
var fdrop_taskedit = new ew_Form("fdrop_taskedit");

// Validate form
fdrop_taskedit.Validate = function() {
...
fdrop_taskedit.Form_CustomValidate = 
...
fdrop_taskedit.ValidateRequired = true;
<?php } else { ?>
fdrop_taskedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdrop_taskedit.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdrop_taskedit.Lists["x_HOSTNAME"] = {"LinkField":"x_server_hostname","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $drop_task_edit->ShowPageHeader(); ?>
<?php
$drop_task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($drop_task_edit->Pager)) $drop_task_edit->Pager = new cNumericPager($drop_task_edit->StartRec, $drop_task_edit->DisplayRecs, $drop_task_edit->TotalRecs, $drop_task_edit->RecRange) ?>
<?php if ($drop_task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($drop_task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_edit->PageUrl() ?>start=<?php echo $drop_task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($drop_task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_edit->PageUrl() ?>start=<?php echo $drop_task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($drop_task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $drop_task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($drop_task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_edit->PageUrl() ?>start=<?php echo $drop_task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($drop_task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_edit->PageUrl() ?>start=<?php echo $drop_task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="fdrop_taskedit" id="fdrop_taskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="drop_task">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_drop_taskedit" class="table table-bordered table-striped">
<?php if ($drop_task->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_drop_task_id"><?php echo $drop_task->id->FldCaption() ?></span></td>
		<td<?php echo $drop_task->id->CellAttributes() ?>>
<span id="el_drop_task_id" class="control-group">
<span<?php echo $drop_task->id->ViewAttributes() ?>>
<?php echo $drop_task->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($drop_task->id->CurrentValue) ?>">
<?php echo $drop_task->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<select data-field="x_server_id_mysqladmin" id="x_server_id_mysqladmin" name="x_server_id_mysqladmin"<?php echo $drop_task->server_id_mysqladmin->EditAttributes() ?>>
<?php
if (is_array($drop_task->server_id_mysqladmin->EditValue)) {
	$arwrk = $drop_task->server_id_mysqladmin->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($drop_task->server_id_mysqladmin->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
// Call Lookup selecting
$drop_task->Lookup_Selecting($drop_task->server_id_mysqladmin, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
...
<select data-field="x_HOSTNAME" id="x_HOSTNAME" name="x_HOSTNAME"<?php echo $drop_task->HOSTNAME->EditAttributes() ?>>
<?php
if (is_array($drop_task->HOSTNAME->EditValue)) {
	$arwrk = $drop_task->HOSTNAME->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($drop_task->HOSTNAME->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
// Call Lookup selecting
$drop_task->Lookup_Selecting($drop_task->HOSTNAME, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
