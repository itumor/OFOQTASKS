...
class cchangepwd extends c_login {
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel($this->ProjectID . 'login');
	...
	function Page_Main() {
		...
		if ($bPostBack && $bValidate) {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					$rsold = $rs->fields;
					if (ew_ComparePassword($rsold['loginpassword'], $this->OldPassword)) {
						$bValidPwd = TRUE;
						$bValidPwd = $this->User_ChangePassword($rsold, $sUsername, $this->OldPassword, $this->NewPassword);
						if ($bValidPwd) {
							$rsnew = array('loginpassword' => $this->NewPassword); // Change Password
							$sEmail = $rsold['Email'];
							$rs->Close();
