...
$parameter_view = NULL; // Initialize page object first

class cparameter_view extends cparameter {
	...
	// Page object name
	var $PageObjName = 'parameter_view';
	...
	function __construct() {
		...
		if (@$_GET["parameter_id"] <> "") {
			$this->RecKey["parameter_id"] = $_GET["parameter_id"];
			$KeyUrl .= "&parameter_id=" . urlencode($this->RecKey["parameter_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["parameter_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["parameter_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "parameterlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "parameterlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_parameter\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_parameter',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fparameterview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($parameter_view)) $parameter_view = new cparameter_view();

// Page init
$parameter_view->Page_Init();

// Page main
$parameter_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$parameter_view->Page_Render();
?>
...
// Page object
var parameter_view = new ew_Page("parameter_view");
parameter_view.PageID = "view"; // Page ID
var EW_PAGE_ID = parameter_view.PageID; // For backward compatibility

// Form object
var fparameterview = new ew_Form("fparameterview");

// Form_CustomValidate event
fparameterview.Form_CustomValidate = 
...
fparameterview.ValidateRequired = true;
<?php } else { ?>
fparameterview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fparameterview.Lists["x_script_id"] = {"LinkField":"x_script_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_script_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $parameter_view->ExportOptions->Render("body") ?>
<?php if (!$parameter_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($parameter_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $parameter_view->ShowPageHeader(); ?>
<?php
$parameter_view->ShowMessage();
?>
<?php if ($parameter->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($parameter_view->Pager)) $parameter_view->Pager = new cNumericPager($parameter_view->StartRec, $parameter_view->DisplayRecs, $parameter_view->TotalRecs, $parameter_view->RecRange) ?>
<?php if ($parameter_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($parameter_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $parameter_view->PageUrl() ?>start=<?php echo $parameter_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($parameter_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $parameter_view->PageUrl() ?>start=<?php echo $parameter_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($parameter_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $parameter_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($parameter_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $parameter_view->PageUrl() ?>start=<?php echo $parameter_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($parameter_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $parameter_view->PageUrl() ?>start=<?php echo $parameter_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="fparameterview" id="fparameterview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="parameter">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_parameterview" class="table table-bordered table-striped">
