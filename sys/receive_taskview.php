...
$receive_task_view = NULL; // Initialize page object first

class creceive_task_view extends creceive_task {
	...
	// Page object name
	var $PageObjName = 'receive_task_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "receive_tasklist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "receive_tasklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_receive_task\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_receive_task',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.freceive_taskview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($receive_task_view)) $receive_task_view = new creceive_task_view();

// Page init
$receive_task_view->Page_Init();

// Page main
$receive_task_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$receive_task_view->Page_Render();
?>
...
// Page object
var receive_task_view = new ew_Page("receive_task_view");
receive_task_view.PageID = "view"; // Page ID
var EW_PAGE_ID = receive_task_view.PageID; // For backward compatibility

// Form object
var freceive_taskview = new ew_Form("freceive_taskview");

// Form_CustomValidate event
freceive_taskview.Form_CustomValidate = 
...
freceive_taskview.ValidateRequired = true;
<?php } else { ?>
freceive_taskview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
freceive_taskview.Lists["x_server_id_sendreceive"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $receive_task_view->ExportOptions->Render("body") ?>
<?php if (!$receive_task_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($receive_task_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $receive_task_view->ShowPageHeader(); ?>
<?php
$receive_task_view->ShowMessage();
?>
<?php if ($receive_task->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($receive_task_view->Pager)) $receive_task_view->Pager = new cNumericPager($receive_task_view->StartRec, $receive_task_view->DisplayRecs, $receive_task_view->TotalRecs, $receive_task_view->RecRange) ?>
<?php if ($receive_task_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($receive_task_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_view->PageUrl() ?>start=<?php echo $receive_task_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($receive_task_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_view->PageUrl() ?>start=<?php echo $receive_task_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($receive_task_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $receive_task_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($receive_task_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_view->PageUrl() ?>start=<?php echo $receive_task_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($receive_task_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_view->PageUrl() ?>start=<?php echo $receive_task_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="freceive_taskview" id="freceive_taskview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="receive_task">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_receive_taskview" class="table table-bordered table-striped">
...
<?php echo $receive_task->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->username->Visible) { // username ?>
	<tr id="r_username">
		<td><span id="elh_receive_task_username"><?php echo $receive_task->username->FldCaption() ?></span></td>
		<td<?php echo $receive_task->username->CellAttributes() ?>>
<span id="el_receive_task_username" class="control-group">
<span<?php echo $receive_task->username->ViewAttributes() ?>>
<?php echo $receive_task->username->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->datetime->Visible) { // datetime ?>
	<tr id="r_datetime">
		<td><span id="elh_receive_task_datetime"><?php echo $receive_task->datetime->FldCaption() ?></span></td>
		<td<?php echo $receive_task->datetime->CellAttributes() ?>>
<span id="el_receive_task_datetime" class="control-group">
<span<?php echo $receive_task->datetime->ViewAttributes() ?>>
<?php echo $receive_task->datetime->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->server_id_sendreceive->Visible) { // server_id_sendreceive ?>
	<tr id="r_server_id_sendreceive">
		<td><span id="elh_receive_task_server_id_sendreceive"><?php echo $receive_task->server_id_sendreceive->FldCaption() ?></span></td>
...
<?php echo $receive_task->server_id_sendreceive->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->TARGET_FILENAME->Visible) { // TARGET_FILENAME ?>
	<tr id="r_TARGET_FILENAME">
		<td><span id="elh_receive_task_TARGET_FILENAME"><?php echo $receive_task->TARGET_FILENAME->FldCaption() ?></span></td>
...
<?php echo $receive_task->TARGET_FILENAME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->LOCAL_PATH->Visible) { // LOCAL_PATH ?>
	<tr id="r_LOCAL_PATH">
		<td><span id="elh_receive_task_LOCAL_PATH"><?php echo $receive_task->LOCAL_PATH->FldCaption() ?></span></td>
...
<?php echo $receive_task->LOCAL_PATH->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_IPAND1STLVL->Visible) { // REMOTE_IPAND1STLVL ?>
	<tr id="r_REMOTE_IPAND1STLVL">
		<td><span id="elh_receive_task_REMOTE_IPAND1STLVL"><?php echo $receive_task->REMOTE_IPAND1STLVL->FldCaption() ?></span></td>
...
<?php echo $receive_task->REMOTE_IPAND1STLVL->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_REMAIN_PATH->Visible) { // REMOTE_REMAIN_PATH ?>
	<tr id="r_REMOTE_REMAIN_PATH">
		<td><span id="elh_receive_task_REMOTE_REMAIN_PATH"><?php echo $receive_task->REMOTE_REMAIN_PATH->FldCaption() ?></span></td>
...
<?php echo $receive_task->REMOTE_REMAIN_PATH->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_USERNAME->Visible) { // REMOTE_USERNAME ?>
	<tr id="r_REMOTE_USERNAME">
		<td><span id="elh_receive_task_REMOTE_USERNAME"><?php echo $receive_task->REMOTE_USERNAME->FldCaption() ?></span></td>
...
<?php echo $receive_task->REMOTE_USERNAME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_PASSWORD->Visible) { // REMOTE_PASSWORD ?>
	<tr id="r_REMOTE_PASSWORD">
		<td><span id="elh_receive_task_REMOTE_PASSWORD"><?php echo $receive_task->REMOTE_PASSWORD->FldCaption() ?></span></td>
...
<?php echo $receive_task->REMOTE_PASSWORD->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_DOMAIN->Visible) { // REMOTE_DOMAIN ?>
	<tr id="r_REMOTE_DOMAIN">
		<td><span id="elh_receive_task_REMOTE_DOMAIN"><?php echo $receive_task->REMOTE_DOMAIN->FldCaption() ?></span></td>
...
<?php echo $receive_task->REMOTE_DOMAIN->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
freceive_taskview.Init();
</script>
<?php
$receive_task_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($receive_task->Export == "") { ?>
...
<?php
$receive_task_view->Page_Terminate();
?>
