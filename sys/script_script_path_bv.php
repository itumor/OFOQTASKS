...
$script_script_path_blobview = NULL; // Initialize page object first

class cscript_script_path_blobview extends cscript {
	...
	// Page object name
	var $PageObjName = 'script_script_path_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'script');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('script', 'x_script_path');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('script_path');
				...
				if (trim(strval($this->Recordset->fields('script_path'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('script_path') . "\"");
				}
...
// Create page object
if (!isset($script_script_path_blobview)) $script_script_path_blobview = new cscript_script_path_blobview();

// Page init
$script_script_path_blobview->Page_Init();

// Page main
$script_script_path_blobview->Page_Main();
?>
<?php
$script_script_path_blobview->Page_Terminate();
?>
