...
$drop_task_view = NULL; // Initialize page object first

class cdrop_task_view extends cdrop_task {
	...
	// Page object name
	var $PageObjName = 'drop_task_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "drop_tasklist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "drop_tasklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_drop_task\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_drop_task',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fdrop_taskview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($drop_task_view)) $drop_task_view = new cdrop_task_view();

// Page init
$drop_task_view->Page_Init();

// Page main
$drop_task_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$drop_task_view->Page_Render();
?>
...
// Page object
var drop_task_view = new ew_Page("drop_task_view");
drop_task_view.PageID = "view"; // Page ID
var EW_PAGE_ID = drop_task_view.PageID; // For backward compatibility

// Form object
var fdrop_taskview = new ew_Form("fdrop_taskview");

// Form_CustomValidate event
fdrop_taskview.Form_CustomValidate = 
...
fdrop_taskview.ValidateRequired = true;
<?php } else { ?>
fdrop_taskview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdrop_taskview.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fdrop_taskview.Lists["x_HOSTNAME"] = {"LinkField":"x_server_hostname","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $drop_task_view->ExportOptions->Render("body") ?>
<?php if (!$drop_task_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($drop_task_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $drop_task_view->ShowPageHeader(); ?>
<?php
$drop_task_view->ShowMessage();
?>
<?php if ($drop_task->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($drop_task_view->Pager)) $drop_task_view->Pager = new cNumericPager($drop_task_view->StartRec, $drop_task_view->DisplayRecs, $drop_task_view->TotalRecs, $drop_task_view->RecRange) ?>
<?php if ($drop_task_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($drop_task_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_view->PageUrl() ?>start=<?php echo $drop_task_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($drop_task_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_view->PageUrl() ?>start=<?php echo $drop_task_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($drop_task_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $drop_task_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($drop_task_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_view->PageUrl() ?>start=<?php echo $drop_task_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($drop_task_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $drop_task_view->PageUrl() ?>start=<?php echo $drop_task_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="fdrop_taskview" id="fdrop_taskview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="drop_task">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_drop_taskview" class="table table-bordered table-striped">
...
<?php echo $drop_task->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($drop_task->server_id_mysqladmin->Visible) { // server_id_mysqladmin ?>
	<tr id="r_server_id_mysqladmin">
		<td><span id="elh_drop_task_server_id_mysqladmin"><?php echo $drop_task->server_id_mysqladmin->FldCaption() ?></span></td>
...
<?php echo $drop_task->server_id_mysqladmin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($drop_task->HOSTNAME->Visible) { // HOSTNAME ?>
	<tr id="r_HOSTNAME">
		<td><span id="elh_drop_task_HOSTNAME"><?php echo $drop_task->HOSTNAME->FldCaption() ?></span></td>
...
<?php echo $drop_task->HOSTNAME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $drop_task->PASSWORD->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($drop_task->DATABASE->Visible) { // DATABASE ?>
	<tr id="r_DATABASE">
		<td><span id="elh_drop_task_DATABASE"><?php echo $drop_task->DATABASE->FldCaption() ?></span></td>
...
<?php echo $drop_task->DATABASE->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
