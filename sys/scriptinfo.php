...
//
// Table class for script
//
class cscript extends cTable {
	var $script_id;
	var $script_name;
	var $script_path;
	var $start_range;
	...
	function __construct() {
		...
		// script_id
		$this->script_id = new cField('script', 'script', 'x_script_id', 'script_id', '`script_id`', '`script_id`', 3, -1, FALSE, '`script_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->script_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['script_id'] = &$this->script_id;

		// script_name
		$this->script_name = new cField('script', 'script', 'x_script_name', 'script_name', '`script_name`', '`script_name`', 200, -1, FALSE, '`script_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['script_name'] = &$this->script_name;

		// script_path
		$this->script_path = new cField('script', 'script', 'x_script_path', 'script_path', '`script_path`', '`script_path`', 200, -1, TRUE, '`script_path`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['script_path'] = &$this->script_path;

		// start_range
		$this->start_range = new cField('script', 'script', 'x_start_range', 'start_range', '`start_range`', '`start_range`', 3, -1, FALSE, '`start_range`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->start_range->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['start_range'] = &$this->start_range;

		// end_range
		$this->end_range = new cField('script', 'script', 'x_end_range', 'end_range', '`end_range`', '`end_range`', 3, -1, FALSE, '`end_range`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('script_id', $rs))
				ew_AddFilter($where, ew_QuotedName('script_id') . '=' . ew_QuotedValue($rs['script_id'], $this->script_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->script_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@script_id@", ew_AdjustSql($this->script_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("scriptview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("scriptview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("scriptedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("scriptadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("scriptdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->script_id->CurrentValue)) {
			$sUrl .= "script_id=" . urlencode($this->script_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["script_id"]; // script_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->script_id->CurrentValue = $key;
