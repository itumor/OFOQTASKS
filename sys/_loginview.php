...
class cp_login_view extends c_login {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["idlogin"] <> "") {
			$this->RecKey["idlogin"] = $_GET["idlogin"];
			$KeyUrl .= "&idlogin=" . urlencode($this->RecKey["idlogin"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["idlogin"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["idlogin"]);
		}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf__login\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf__login',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.f_loginview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
<table class="ewPager">
<tr><td>
<?php if (!isset($p_login_view->Pager)) $p_login_view->Pager = new cNumericPager($p_login_view->StartRec, $p_login_view->DisplayRecs, $p_login_view->TotalRecs, $p_login_view->RecRange) ?>
<?php if ($p_login_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($p_login_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $p_login_view->PageUrl() ?>start=<?php echo $p_login_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($p_login_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $p_login_view->PageUrl() ?>start=<?php echo $p_login_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($p_login_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $p_login_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($p_login_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $p_login_view->PageUrl() ?>start=<?php echo $p_login_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($p_login_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $p_login_view->PageUrl() ?>start=<?php echo $p_login_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
