...
$restore_task_view = NULL; // Initialize page object first

class crestore_task_view extends crestore_task {
	...
	// Page object name
	var $PageObjName = 'restore_task_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "restore_tasklist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "restore_tasklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_restore_task\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_restore_task',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.frestore_taskview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($restore_task_view)) $restore_task_view = new crestore_task_view();

// Page init
$restore_task_view->Page_Init();

// Page main
$restore_task_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$restore_task_view->Page_Render();
?>
...
// Page object
var restore_task_view = new ew_Page("restore_task_view");
restore_task_view.PageID = "view"; // Page ID
var EW_PAGE_ID = restore_task_view.PageID; // For backward compatibility

// Form object
var frestore_taskview = new ew_Form("frestore_taskview");

// Form_CustomValidate event
frestore_taskview.Form_CustomValidate = 
...
frestore_taskview.ValidateRequired = true;
<?php } else { ?>
frestore_taskview.ValidateRequired = false; 
<?php } ?>
...
<?php $restore_task_view->ExportOptions->Render("body") ?>
<?php if (!$restore_task_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($restore_task_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $restore_task_view->ShowPageHeader(); ?>
<?php
$restore_task_view->ShowMessage();
?>
<?php if ($restore_task->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($restore_task_view->Pager)) $restore_task_view->Pager = new cNumericPager($restore_task_view->StartRec, $restore_task_view->DisplayRecs, $restore_task_view->TotalRecs, $restore_task_view->RecRange) ?>
<?php if ($restore_task_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($restore_task_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $restore_task_view->PageUrl() ?>start=<?php echo $restore_task_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($restore_task_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $restore_task_view->PageUrl() ?>start=<?php echo $restore_task_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($restore_task_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $restore_task_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($restore_task_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $restore_task_view->PageUrl() ?>start=<?php echo $restore_task_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($restore_task_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $restore_task_view->PageUrl() ?>start=<?php echo $restore_task_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="frestore_taskview" id="frestore_taskview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="restore_task">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_restore_taskview" class="table table-bordered table-striped">
...
<?php echo $restore_task->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->server_id_mysqladmin->Visible) { // server_id_mysqladmin ?>
	<tr id="r_server_id_mysqladmin">
		<td><span id="elh_restore_task_server_id_mysqladmin"><?php echo $restore_task->server_id_mysqladmin->FldCaption() ?></span></td>
...
<?php echo $restore_task->server_id_mysqladmin->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->HOSTNAME->Visible) { // HOSTNAME ?>
	<tr id="r_HOSTNAME">
		<td><span id="elh_restore_task_HOSTNAME"><?php echo $restore_task->HOSTNAME->FldCaption() ?></span></td>
...
<?php echo $restore_task->HOSTNAME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->PASSWORD->Visible) { // PASSWORD ?>
	<tr id="r_PASSWORD">
		<td><span id="elh_restore_task_PASSWORD"><?php echo $restore_task->PASSWORD->FldCaption() ?></span></td>
...
<?php echo $restore_task->PASSWORD->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->DATABASE->Visible) { // DATABASE ?>
	<tr id="r_DATABASE">
		<td><span id="elh_restore_task_DATABASE"><?php echo $restore_task->DATABASE->FldCaption() ?></span></td>
...
<?php echo $restore_task->DATABASE->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->FILEPATH->Visible) { // FILEPATH ?>
	<tr id="r_FILEPATH">
		<td><span id="elh_restore_task_FILEPATH"><?php echo $restore_task->FILEPATH->FldCaption() ?></span></td>
...
<?php echo $restore_task->FILEPATH->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->FILENAME->Visible) { // FILENAME ?>
	<tr id="r_FILENAME">
		<td><span id="elh_restore_task_FILENAME"><?php echo $restore_task->FILENAME->FldCaption() ?></span></td>
...
<?php if ($restore_task->datetime->Visible) { // datetime ?>
	<tr id="r_datetime">
		<td><span id="elh_restore_task_datetime"><?php echo $restore_task->datetime->FldCaption() ?></span></td>
...
<?php echo $restore_task->datetime->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->DBUSERNAME->Visible) { // DBUSERNAME ?>
	<tr id="r_DBUSERNAME">
		<td><span id="elh_restore_task_DBUSERNAME"><?php echo $restore_task->DBUSERNAME->FldCaption() ?></span></td>
...
<?php echo $restore_task->DBUSERNAME->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($restore_task->username->Visible) { // username ?>
	<tr id="r_username">
		<td><span id="elh_restore_task_username"><?php echo $restore_task->username->FldCaption() ?></span></td>
...
<?php echo $restore_task->username->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<script type="text/javascript">
frestore_taskview.Init();
</script>
<?php
$restore_task_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($restore_task->Export == "") { ?>
...
<?php
$restore_task_view->Page_Terminate();
?>
