...
$stop_task_edit = NULL; // Initialize page object first

class cstop_task_edit extends cstop_task {
	...
	// Page object name
	var $PageObjName = 'stop_task_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("stop_tasklist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "stop_taskview.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "stop_tasklist.php", $this->TableVar);
...
// Create page object
if (!isset($stop_task_edit)) $stop_task_edit = new cstop_task_edit();

// Page init
$stop_task_edit->Page_Init();

// Page main
$stop_task_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$stop_task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var stop_task_edit = new ew_Page("stop_task_edit");
stop_task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = stop_task_edit.PageID; // For backward compatibility

// Form object
var fstop_taskedit = new ew_Form("fstop_taskedit");

// Validate form
fstop_taskedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($stop_task->HOSTNAME->FldCaption()) ?>");
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($stop_task->PASSWORD->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_DATABASE");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($stop_task->DATABASE->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_FILEPATH");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($stop_task->FILEPATH->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_FILENAME");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($stop_task->FILENAME->FldCaption()) ?>");
...
fstop_taskedit.Form_CustomValidate = 
...
fstop_taskedit.ValidateRequired = true;
<?php } else { ?>
fstop_taskedit.ValidateRequired = false; 
<?php } ?>
...
<?php $stop_task_edit->ShowPageHeader(); ?>
<?php
$stop_task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($stop_task_edit->Pager)) $stop_task_edit->Pager = new cNumericPager($stop_task_edit->StartRec, $stop_task_edit->DisplayRecs, $stop_task_edit->TotalRecs, $stop_task_edit->RecRange) ?>
<?php if ($stop_task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($stop_task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $stop_task_edit->PageUrl() ?>start=<?php echo $stop_task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($stop_task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $stop_task_edit->PageUrl() ?>start=<?php echo $stop_task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($stop_task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $stop_task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($stop_task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $stop_task_edit->PageUrl() ?>start=<?php echo $stop_task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($stop_task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $stop_task_edit->PageUrl() ?>start=<?php echo $stop_task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="fstop_taskedit" id="fstop_taskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="stop_task">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_stop_taskedit" class="table table-bordered table-striped">
<?php if ($stop_task->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_stop_task_id"><?php echo $stop_task->id->FldCaption() ?></span></td>
		<td<?php echo $stop_task->id->CellAttributes() ?>>
<span id="el_stop_task_id" class="control-group">
<span<?php echo $stop_task->id->ViewAttributes() ?>>
<?php echo $stop_task->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($stop_task->id->CurrentValue) ?>">
<?php echo $stop_task->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if ($stop_task->HOSTNAME->Visible) { // HOSTNAME ?>
	<tr id="r_HOSTNAME">
		<td><span id="elh_stop_task_HOSTNAME"><?php echo $stop_task->HOSTNAME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $stop_task->HOSTNAME->CellAttributes() ?>>
<span id="el_stop_task_HOSTNAME" class="control-group">
<input type="text" data-field="x_HOSTNAME" name="x_HOSTNAME" id="x_HOSTNAME" size="30" maxlength="255" placeholder="<?php echo $stop_task->HOSTNAME->PlaceHolder ?>" value="<?php echo $stop_task->HOSTNAME->EditValue ?>"<?php echo $stop_task->HOSTNAME->EditAttributes() ?>>
</span>
<?php echo $stop_task->HOSTNAME->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if ($stop_task->PASSWORD->Visible) { // PASSWORD ?>
	<tr id="r_PASSWORD">
		<td><span id="elh_stop_task_PASSWORD"><?php echo $stop_task->PASSWORD->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $stop_task->PASSWORD->CellAttributes() ?>>
<span id="el_stop_task_PASSWORD" class="control-group">
<input type="text" data-field="x_PASSWORD" name="x_PASSWORD" id="x_PASSWORD" size="30" maxlength="255" placeholder="<?php echo $stop_task->PASSWORD->PlaceHolder ?>" value="<?php echo $stop_task->PASSWORD->EditValue ?>"<?php echo $stop_task->PASSWORD->EditAttributes() ?>>
</span>
<?php echo $stop_task->PASSWORD->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($stop_task->DATABASE->Visible) { // DATABASE ?>
	<tr id="r_DATABASE">
		<td><span id="elh_stop_task_DATABASE"><?php echo $stop_task->DATABASE->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $stop_task->DATABASE->CellAttributes() ?>>
<span id="el_stop_task_DATABASE" class="control-group">
<input type="text" data-field="x_DATABASE" name="x_DATABASE" id="x_DATABASE" size="30" maxlength="255" placeholder="<?php echo $stop_task->DATABASE->PlaceHolder ?>" value="<?php echo $stop_task->DATABASE->EditValue ?>"<?php echo $stop_task->DATABASE->EditAttributes() ?>>
</span>
<?php echo $stop_task->DATABASE->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($stop_task->FILEPATH->Visible) { // FILEPATH ?>
	<tr id="r_FILEPATH">
		<td><span id="elh_stop_task_FILEPATH"><?php echo $stop_task->FILEPATH->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $stop_task->FILEPATH->CellAttributes() ?>>
<span id="el_stop_task_FILEPATH" class="control-group">
<input type="text" data-field="x_FILEPATH" name="x_FILEPATH" id="x_FILEPATH" size="30" maxlength="255" placeholder="<?php echo $stop_task->FILEPATH->PlaceHolder ?>" value="<?php echo $stop_task->FILEPATH->EditValue ?>"<?php echo $stop_task->FILEPATH->EditAttributes() ?>>
</span>
<?php echo $stop_task->FILEPATH->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($stop_task->FILENAME->Visible) { // FILENAME ?>
	<tr id="r_FILENAME">
		<td><span id="elh_stop_task_FILENAME"><?php echo $stop_task->FILENAME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $stop_task->FILENAME->CellAttributes() ?>>
<span id="el_stop_task_FILENAME" class="control-group">
<input type="text" data-field="x_FILENAME" name="x_FILENAME" id="x_FILENAME" size="30" maxlength="255" placeholder="<?php echo $stop_task->FILENAME->PlaceHolder ?>" value="<?php echo $stop_task->FILENAME->EditValue ?>"<?php echo $stop_task->FILENAME->EditAttributes() ?>>
</span>
<?php echo $stop_task->FILENAME->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
