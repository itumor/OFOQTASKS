...
$server_server_file_blobview = NULL; // Initialize page object first

class cserver_server_file_blobview extends cserver {
	...
	// Page object name
	var $PageObjName = 'server_server_file_blobview';
	...
	function Page_Init() {
		...
		$Security->LoadCurrentUserLevel(CurrentProjectID() . 'server');
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('server', 'x_server_file');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('server_file');
				...
				if (trim(strval($this->Recordset->fields('server_file'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('server_file') . "\"");
				}
...
// Create page object
if (!isset($server_server_file_blobview)) $server_server_file_blobview = new cserver_server_file_blobview();

// Page init
$server_server_file_blobview->Page_Init();

// Page main
$server_server_file_blobview->Page_Main();
?>
<?php
$server_server_file_blobview->Page_Terminate();
?>
