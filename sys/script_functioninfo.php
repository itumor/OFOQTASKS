...
//
// Table class for script_function
//
class cscript_function extends cTable {
	...
	function __construct() {
		...
		// script_function_id
		$this->script_function_id = new cField('script_function', 'script_function', 'x_script_function_id', 'script_function_id', '`script_function_id`', '`script_function_id`', 3, -1, FALSE, '`script_function_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		$this->script_function_name = new cField('script_function', 'script_function', 'x_script_function_name', 'script_function_name', '`script_function_name`', '`script_function_name`', 200, -1, FALSE, '`script_function_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['script_function_name'] = &$this->script_function_name;

		// script_id
		$this->script_id = new cField('script_function', 'script_function', 'x_script_id', 'script_id', '`script_id`', '`script_id`', 3, -1, FALSE, '`script_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	// Update Table
	var $UpdateTable = "`script_function`";
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('script_function_id', $rs))
				ew_AddFilter($where, ew_QuotedName('script_function_id') . '=' . ew_QuotedValue($rs['script_function_id'], $this->script_function_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`script_function_id` = @script_function_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->script_function_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@script_function_id@", ew_AdjustSql($this->script_function_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "script_functionlist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("script_functionview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("script_functionview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "script_functionadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("script_functionedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("script_functionadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("script_functiondelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->script_function_id->CurrentValue)) {
			$sUrl .= "script_function_id=" . urlencode($this->script_function_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["script_function_id"]; // script_function_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->script_function_id->CurrentValue = $key;
