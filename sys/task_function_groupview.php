...
$task_function_group_view = NULL; // Initialize page object first

class ctask_function_group_view extends ctask_function_group {
	...
	// Page object name
	var $PageObjName = 'task_function_group_view';
	...
	function __construct() {
		...
		if (@$_GET["task_function_group_id"] <> "") {
			$this->RecKey["task_function_group_id"] = $_GET["task_function_group_id"];
			$KeyUrl .= "&task_function_group_id=" . urlencode($this->RecKey["task_function_group_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["task_function_group_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["task_function_group_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "task_function_grouplist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "task_function_grouplist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($task_function_group_view)) $task_function_group_view = new ctask_function_group_view();

// Page init
$task_function_group_view->Page_Init();

// Page main
$task_function_group_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$task_function_group_view->Page_Render();
?>
...
// Page object
var task_function_group_view = new ew_Page("task_function_group_view");
task_function_group_view.PageID = "view"; // Page ID
var EW_PAGE_ID = task_function_group_view.PageID; // For backward compatibility

// Form object
var ftask_function_groupview = new ew_Form("ftask_function_groupview");

// Form_CustomValidate event
ftask_function_groupview.Form_CustomValidate = 
...
ftask_function_groupview.ValidateRequired = true;
<?php } else { ?>
ftask_function_groupview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ftask_function_groupview.Lists["x_task_id"] = {"LinkField":"x_task_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_task_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
ftask_function_groupview.Lists["x_function_group_id"] = {"LinkField":"x_function_group_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_function_group_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $task_function_group_view->ExportOptions->Render("body") ?>
<?php if (!$task_function_group_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($task_function_group_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $task_function_group_view->ShowPageHeader(); ?>
<?php
$task_function_group_view->ShowMessage();
?>
<?php if ($task_function_group->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($task_function_group_view->Pager)) $task_function_group_view->Pager = new cNumericPager($task_function_group_view->StartRec, $task_function_group_view->DisplayRecs, $task_function_group_view->TotalRecs, $task_function_group_view->RecRange) ?>
<?php if ($task_function_group_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($task_function_group_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $task_function_group_view->PageUrl() ?>start=<?php echo $task_function_group_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($task_function_group_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $task_function_group_view->PageUrl() ?>start=<?php echo $task_function_group_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($task_function_group_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $task_function_group_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($task_function_group_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $task_function_group_view->PageUrl() ?>start=<?php echo $task_function_group_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($task_function_group_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $task_function_group_view->PageUrl() ?>start=<?php echo $task_function_group_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="ftask_function_groupview" id="ftask_function_groupview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="task_function_group">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_task_function_groupview" class="table table-bordered table-striped">
...
<script type="text/javascript">
ftask_function_groupview.Init();
</script>
<?php
$task_function_group_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($task_function_group->Export == "") { ?>
...
<?php
$task_function_group_view->Page_Terminate();
?>
