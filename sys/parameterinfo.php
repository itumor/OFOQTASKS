...
//
// Table class for parameter
//
class cparameter extends cTable {
	var $parameter_id;
	var $script_id;
	var $parameter_name;
	...
	function __construct() {
		...
		// parameter_id
		$this->parameter_id = new cField('parameter', 'parameter', 'x_parameter_id', 'parameter_id', '`parameter_id`', '`parameter_id`', 3, -1, FALSE, '`parameter_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->parameter_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['parameter_id'] = &$this->parameter_id;

		// script_id
		$this->script_id = new cField('parameter', 'parameter', 'x_script_id', 'script_id', '`script_id`', '`script_id`', 3, -1, FALSE, '`script_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->script_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['script_id'] = &$this->script_id;

		// parameter_name
		$this->parameter_name = new cField('parameter', 'parameter', 'x_parameter_name', 'parameter_name', '`parameter_name`', '`parameter_name`', 200, -1, FALSE, '`parameter_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('parameter_id', $rs))
				ew_AddFilter($where, ew_QuotedName('parameter_id') . '=' . ew_QuotedValue($rs['parameter_id'], $this->parameter_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->parameter_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@parameter_id@", ew_AdjustSql($this->parameter_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("parameterview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("parameterview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "parameteradd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("parameteredit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("parameteradd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("parameterdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->parameter_id->CurrentValue)) {
			$sUrl .= "parameter_id=" . urlencode($this->parameter_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["parameter_id"]; // parameter_id
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->parameter_id->CurrentValue = $key;
