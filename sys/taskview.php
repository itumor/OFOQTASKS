...
$task_view = NULL; // Initialize page object first

class ctask_view extends ctask {
	...
	function Page_Init() {
		...
		if (@$_GET["task_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["task_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("tasklist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->task_id->CurrentValue) == strval($this->Recordset->fields('task_id'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "tasklist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "tasklist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_task\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_task',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.ftaskview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($task_view)) $task_view = new ctask_view();
...
// Page object
var task_view = new ew_Page("task_view");
task_view.PageID = "view"; // Page ID
var EW_PAGE_ID = task_view.PageID; // For backward compatibility

// Form object
var ftaskview = new ew_Form("ftaskview");

// Form_CustomValidate event
ftaskview.Form_CustomValidate = 
...
ftaskview.ValidateRequired = true;
<?php } else { ?>
ftaskview.ValidateRequired = false; 
<?php } ?>
...
<?php $task_view->ExportOptions->Render("body") ?>
<?php if (!$task_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($task_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $task_view->ShowPageHeader(); ?>
<?php
$task_view->ShowMessage();
?>
<?php if ($task->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($task_view->Pager)) $task_view->Pager = new cNumericPager($task_view->StartRec, $task_view->DisplayRecs, $task_view->TotalRecs, $task_view->RecRange) ?>
<?php if ($task_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($task_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $task_view->PageUrl() ?>start=<?php echo $task_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($task_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $task_view->PageUrl() ?>start=<?php echo $task_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($task_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $task_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($task_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $task_view->PageUrl() ?>start=<?php echo $task_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($task_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $task_view->PageUrl() ?>start=<?php echo $task_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="ftaskview" id="ftaskview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="task">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_taskview" class="table table-bordered table-striped">
...
<?php echo $task->task_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
