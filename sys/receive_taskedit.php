...
$receive_task_edit = NULL; // Initialize page object first

class creceive_task_edit extends creceive_task {
	...
	// Page object name
	var $PageObjName = 'receive_task_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("receive_tasklist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "receive_taskview.php")
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (trim(strval($this->server_id_sendreceive->CurrentValue)) == "") {
				$sFilterWrk = "0=1";
			} else {
				$sFilterWrk = "`server_id`" . ew_SearchString("=", $this->server_id_sendreceive->CurrentValue, EW_DATATYPE_NUMBER);
			}
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "receive_tasklist.php", $this->TableVar);
...
// Create page object
if (!isset($receive_task_edit)) $receive_task_edit = new creceive_task_edit();

// Page init
$receive_task_edit->Page_Init();

// Page main
$receive_task_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$receive_task_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var receive_task_edit = new ew_Page("receive_task_edit");
receive_task_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = receive_task_edit.PageID; // For backward compatibility

// Form object
var freceive_taskedit = new ew_Form("freceive_taskedit");

// Validate form
freceive_taskedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->server_id_sendreceive->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_TARGET_FILENAME");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->TARGET_FILENAME->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_LOCAL_PATH");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->LOCAL_PATH->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_REMOTE_IPAND1STLVL");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->REMOTE_IPAND1STLVL->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_REMOTE_REMAIN_PATH");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->REMOTE_REMAIN_PATH->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_REMOTE_USERNAME");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->REMOTE_USERNAME->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_REMOTE_PASSWORD");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->REMOTE_PASSWORD->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_REMOTE_DOMAIN");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($receive_task->REMOTE_DOMAIN->FldCaption()) ?>");
...
freceive_taskedit.Form_CustomValidate = 
...
freceive_taskedit.ValidateRequired = true;
<?php } else { ?>
freceive_taskedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
freceive_taskedit.Lists["x_server_id_sendreceive"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $receive_task_edit->ShowPageHeader(); ?>
<?php
$receive_task_edit->ShowMessage();
?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($receive_task_edit->Pager)) $receive_task_edit->Pager = new cNumericPager($receive_task_edit->StartRec, $receive_task_edit->DisplayRecs, $receive_task_edit->TotalRecs, $receive_task_edit->RecRange) ?>
<?php if ($receive_task_edit->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($receive_task_edit->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_edit->PageUrl() ?>start=<?php echo $receive_task_edit->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($receive_task_edit->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_edit->PageUrl() ?>start=<?php echo $receive_task_edit->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($receive_task_edit->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $receive_task_edit->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($receive_task_edit->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_edit->PageUrl() ?>start=<?php echo $receive_task_edit->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($receive_task_edit->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $receive_task_edit->PageUrl() ?>start=<?php echo $receive_task_edit->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<form name="freceive_taskedit" id="freceive_taskedit" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="receive_task">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_receive_taskedit" class="table table-bordered table-striped">
<?php if ($receive_task->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_receive_task_id"><?php echo $receive_task->id->FldCaption() ?></span></td>
		<td<?php echo $receive_task->id->CellAttributes() ?>>
<span id="el_receive_task_id" class="control-group">
<span<?php echo $receive_task->id->ViewAttributes() ?>>
<?php echo $receive_task->id->EditValue ?></span>
</span>
<input type="hidden" data-field="x_id" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($receive_task->id->CurrentValue) ?>">
<?php echo $receive_task->id->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->server_id_sendreceive->Visible) { // server_id_sendreceive ?>
	<tr id="r_server_id_sendreceive">
		<td><span id="elh_receive_task_server_id_sendreceive"><?php echo $receive_task->server_id_sendreceive->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->server_id_sendreceive->CellAttributes() ?>>
<span id="el_receive_task_server_id_sendreceive" class="control-group">
<select data-field="x_server_id_sendreceive" id="x_server_id_sendreceive" name="x_server_id_sendreceive"<?php echo $receive_task->server_id_sendreceive->EditAttributes() ?>>
<?php
if (is_array($receive_task->server_id_sendreceive->EditValue)) {
	$arwrk = $receive_task->server_id_sendreceive->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($receive_task->server_id_sendreceive->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
// Call Lookup selecting
$receive_task->Lookup_Selecting($receive_task->server_id_sendreceive, $sWhereWrk);
if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
?>
<input type="hidden" name="s_x_server_id_sendreceive" id="s_x_server_id_sendreceive" value="s=<?php echo ew_Encrypt($sSqlWrk) ?>&f0=<?php echo ew_Encrypt("`server_id` = {filter_value}"); ?>&t0=3">
</span>
<?php echo $receive_task->server_id_sendreceive->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->TARGET_FILENAME->Visible) { // TARGET_FILENAME ?>
	<tr id="r_TARGET_FILENAME">
		<td><span id="elh_receive_task_TARGET_FILENAME"><?php echo $receive_task->TARGET_FILENAME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->TARGET_FILENAME->CellAttributes() ?>>
<span id="el_receive_task_TARGET_FILENAME" class="control-group">
<input type="text" data-field="x_TARGET_FILENAME" name="x_TARGET_FILENAME" id="x_TARGET_FILENAME" size="30" maxlength="255" placeholder="<?php echo $receive_task->TARGET_FILENAME->PlaceHolder ?>" value="<?php echo $receive_task->TARGET_FILENAME->EditValue ?>"<?php echo $receive_task->TARGET_FILENAME->EditAttributes() ?>>
</span>
<?php echo $receive_task->TARGET_FILENAME->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->LOCAL_PATH->Visible) { // LOCAL_PATH ?>
	<tr id="r_LOCAL_PATH">
		<td><span id="elh_receive_task_LOCAL_PATH"><?php echo $receive_task->LOCAL_PATH->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->LOCAL_PATH->CellAttributes() ?>>
<span id="el_receive_task_LOCAL_PATH" class="control-group">
<input type="text" data-field="x_LOCAL_PATH" name="x_LOCAL_PATH" id="x_LOCAL_PATH" size="30" maxlength="255" placeholder="<?php echo $receive_task->LOCAL_PATH->PlaceHolder ?>" value="<?php echo $receive_task->LOCAL_PATH->EditValue ?>"<?php echo $receive_task->LOCAL_PATH->EditAttributes() ?>>
</span>
<?php echo $receive_task->LOCAL_PATH->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_IPAND1STLVL->Visible) { // REMOTE_IPAND1STLVL ?>
	<tr id="r_REMOTE_IPAND1STLVL">
		<td><span id="elh_receive_task_REMOTE_IPAND1STLVL"><?php echo $receive_task->REMOTE_IPAND1STLVL->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->REMOTE_IPAND1STLVL->CellAttributes() ?>>
<span id="el_receive_task_REMOTE_IPAND1STLVL" class="control-group">
<input type="text" data-field="x_REMOTE_IPAND1STLVL" name="x_REMOTE_IPAND1STLVL" id="x_REMOTE_IPAND1STLVL" size="30" maxlength="255" placeholder="<?php echo $receive_task->REMOTE_IPAND1STLVL->PlaceHolder ?>" value="<?php echo $receive_task->REMOTE_IPAND1STLVL->EditValue ?>"<?php echo $receive_task->REMOTE_IPAND1STLVL->EditAttributes() ?>>
</span>
<?php echo $receive_task->REMOTE_IPAND1STLVL->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_REMAIN_PATH->Visible) { // REMOTE_REMAIN_PATH ?>
	<tr id="r_REMOTE_REMAIN_PATH">
		<td><span id="elh_receive_task_REMOTE_REMAIN_PATH"><?php echo $receive_task->REMOTE_REMAIN_PATH->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->REMOTE_REMAIN_PATH->CellAttributes() ?>>
<span id="el_receive_task_REMOTE_REMAIN_PATH" class="control-group">
<input type="text" data-field="x_REMOTE_REMAIN_PATH" name="x_REMOTE_REMAIN_PATH" id="x_REMOTE_REMAIN_PATH" size="30" maxlength="255" placeholder="<?php echo $receive_task->REMOTE_REMAIN_PATH->PlaceHolder ?>" value="<?php echo $receive_task->REMOTE_REMAIN_PATH->EditValue ?>"<?php echo $receive_task->REMOTE_REMAIN_PATH->EditAttributes() ?>>
</span>
<?php echo $receive_task->REMOTE_REMAIN_PATH->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_USERNAME->Visible) { // REMOTE_USERNAME ?>
	<tr id="r_REMOTE_USERNAME">
		<td><span id="elh_receive_task_REMOTE_USERNAME"><?php echo $receive_task->REMOTE_USERNAME->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->REMOTE_USERNAME->CellAttributes() ?>>
<span id="el_receive_task_REMOTE_USERNAME" class="control-group">
<input type="text" data-field="x_REMOTE_USERNAME" name="x_REMOTE_USERNAME" id="x_REMOTE_USERNAME" size="30" maxlength="255" placeholder="<?php echo $receive_task->REMOTE_USERNAME->PlaceHolder ?>" value="<?php echo $receive_task->REMOTE_USERNAME->EditValue ?>"<?php echo $receive_task->REMOTE_USERNAME->EditAttributes() ?>>
</span>
<?php echo $receive_task->REMOTE_USERNAME->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_PASSWORD->Visible) { // REMOTE_PASSWORD ?>
	<tr id="r_REMOTE_PASSWORD">
		<td><span id="elh_receive_task_REMOTE_PASSWORD"><?php echo $receive_task->REMOTE_PASSWORD->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->REMOTE_PASSWORD->CellAttributes() ?>>
<span id="el_receive_task_REMOTE_PASSWORD" class="control-group">
<input type="text" data-field="x_REMOTE_PASSWORD" name="x_REMOTE_PASSWORD" id="x_REMOTE_PASSWORD" size="30" maxlength="255" placeholder="<?php echo $receive_task->REMOTE_PASSWORD->PlaceHolder ?>" value="<?php echo $receive_task->REMOTE_PASSWORD->EditValue ?>"<?php echo $receive_task->REMOTE_PASSWORD->EditAttributes() ?>>
</span>
<?php echo $receive_task->REMOTE_PASSWORD->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($receive_task->REMOTE_DOMAIN->Visible) { // REMOTE_DOMAIN ?>
	<tr id="r_REMOTE_DOMAIN">
		<td><span id="elh_receive_task_REMOTE_DOMAIN"><?php echo $receive_task->REMOTE_DOMAIN->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $receive_task->REMOTE_DOMAIN->CellAttributes() ?>>
<span id="el_receive_task_REMOTE_DOMAIN" class="control-group">
<input type="text" data-field="x_REMOTE_DOMAIN" name="x_REMOTE_DOMAIN" id="x_REMOTE_DOMAIN" size="30" maxlength="255" placeholder="<?php echo $receive_task->REMOTE_DOMAIN->PlaceHolder ?>" value="<?php echo $receive_task->REMOTE_DOMAIN->EditValue ?>"<?php echo $receive_task->REMOTE_DOMAIN->EditAttributes() ?>>
</span>
<?php echo $receive_task->REMOTE_DOMAIN->CustomMsg ?></td>
	</tr>
<?php } ?>
</table>
</td></tr></table>
...
<script type="text/javascript">
freceive_taskedit.Init();
<?php if (EW_MOBILE_REFLOW && ew_IsMobile()) { ?>
ew_Reflow();
<?php } ?>
</script>
<?php
$receive_task_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$receive_task_edit->Page_Terminate();
?>
