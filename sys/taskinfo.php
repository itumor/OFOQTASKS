...
class ctask extends cTable {
	...
	function __construct() {
		...
		// task_id
		$this->task_id = new cField('task', 'task', 'x_task_id', 'task_id', '`task_id`', '`task_id`', 3, -1, FALSE, '`task_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('task_id', $rs))
				ew_AddFilter($where, ew_QuotedName('task_id') . '=' . ew_QuotedValue($rs['task_id'], $this->task_id->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@task_id@", ew_AdjustSql($this->task_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("taskview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("taskview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("taskedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("taskadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("taskdelete.php", $this->UrlParm());
	}
