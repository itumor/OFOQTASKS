...
$restore_task_delete = NULL; // Initialize page object first

class crestore_task_delete extends crestore_task {
	...
	// Page object name
	var $PageObjName = 'restore_task_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("restore_tasklist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in restore_task class, restore_taskinfo.php
	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				@unlink(ew_UploadPathEx(TRUE, $this->FILENAME->OldUploadPath) . $row['FILENAME']);
				$conn->raiseErrorFn = 'ew_ErrorFn';
...
// Create page object
if (!isset($restore_task_delete)) $restore_task_delete = new crestore_task_delete();

// Page init
$restore_task_delete->Page_Init();

// Page main
$restore_task_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$restore_task_delete->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var restore_task_delete = new ew_Page("restore_task_delete");
restore_task_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = restore_task_delete.PageID; // For backward compatibility

// Form object
var frestore_taskdelete = new ew_Form("frestore_taskdelete");

// Form_CustomValidate event
frestore_taskdelete.Form_CustomValidate = 
...
frestore_taskdelete.ValidateRequired = true;
<?php } else { ?>
frestore_taskdelete.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
frestore_taskdelete.Lists["x_server_id_mysqladmin"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
frestore_taskdelete.Lists["x_HOSTNAME"] = {"LinkField":"x_server_hostname","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
if ($restore_task_delete->Recordset = $restore_task_delete->LoadRecordset())
	$restore_task_deleteTotalRecs = $restore_task_delete->Recordset->RecordCount(); // Get record count
if ($restore_task_deleteTotalRecs <= 0) { // No record found, exit
	if ($restore_task_delete->Recordset)
		$restore_task_delete->Recordset->Close();
	$restore_task_delete->Page_Terminate("restore_tasklist.php"); // Return to list
}
?>
<?php $Breadcrumb->Render(); ?>
<?php $restore_task_delete->ShowPageHeader(); ?>
<?php
$restore_task_delete->ShowMessage();
?>
<form name="frestore_taskdelete" id="frestore_taskdelete" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="restore_task">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($restore_task_delete->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_restore_taskdelete" class="ewTable ewTableSeparate">
<?php echo $restore_task->TableCustomInnerHtml ?>
	<thead>
	<tr class="ewTableHeader">
<?php if ($restore_task->id->Visible) { // id ?>
		<td><span id="elh_restore_task_id" class="restore_task_id"><?php echo $restore_task->id->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->server_id_mysqladmin->Visible) { // server_id_mysqladmin ?>
		<td><span id="elh_restore_task_server_id_mysqladmin" class="restore_task_server_id_mysqladmin"><?php echo $restore_task->server_id_mysqladmin->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->HOSTNAME->Visible) { // HOSTNAME ?>
		<td><span id="elh_restore_task_HOSTNAME" class="restore_task_HOSTNAME"><?php echo $restore_task->HOSTNAME->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->PASSWORD->Visible) { // PASSWORD ?>
		<td><span id="elh_restore_task_PASSWORD" class="restore_task_PASSWORD"><?php echo $restore_task->PASSWORD->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->DATABASE->Visible) { // DATABASE ?>
		<td><span id="elh_restore_task_DATABASE" class="restore_task_DATABASE"><?php echo $restore_task->DATABASE->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->FILEPATH->Visible) { // FILEPATH ?>
		<td><span id="elh_restore_task_FILEPATH" class="restore_task_FILEPATH"><?php echo $restore_task->FILEPATH->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->FILENAME->Visible) { // FILENAME ?>
		<td><span id="elh_restore_task_FILENAME" class="restore_task_FILENAME"><?php echo $restore_task->FILENAME->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->datetime->Visible) { // datetime ?>
		<td><span id="elh_restore_task_datetime" class="restore_task_datetime"><?php echo $restore_task->datetime->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->DBUSERNAME->Visible) { // DBUSERNAME ?>
		<td><span id="elh_restore_task_DBUSERNAME" class="restore_task_DBUSERNAME"><?php echo $restore_task->DBUSERNAME->FldCaption() ?></span></td>
<?php } ?>
<?php if ($restore_task->username->Visible) { // username ?>
		<td><span id="elh_restore_task_username" class="restore_task_username"><?php echo $restore_task->username->FldCaption() ?></span></td>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$restore_task_delete->RecCnt = 0;
$i = 0;
while (!$restore_task_delete->Recordset->EOF) {
	$restore_task_delete->RecCnt++;
	$restore_task_delete->RowCnt++;

	// Set row properties
	$restore_task->ResetAttrs();
	$restore_task->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$restore_task_delete->LoadRowValues($restore_task_delete->Recordset);

	// Render row
	$restore_task_delete->RenderRow();
?>
	<tr<?php echo $restore_task->RowAttributes() ?>>
<?php if ($restore_task->id->Visible) { // id ?>
		<td<?php echo $restore_task->id->CellAttributes() ?>>
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_id" class="control-group restore_task_id">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_server_id_mysqladmin" class="control-group restore_task_server_id_mysqladmin">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_HOSTNAME" class="control-group restore_task_HOSTNAME">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_PASSWORD" class="control-group restore_task_PASSWORD">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_DATABASE" class="control-group restore_task_DATABASE">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_FILEPATH" class="control-group restore_task_FILEPATH">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_FILENAME" class="control-group restore_task_FILENAME">
...
<?php echo $restore_task->FILENAME->ListViewValue() ?>
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_datetime" class="control-group restore_task_datetime">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_DBUSERNAME" class="control-group restore_task_DBUSERNAME">
...
<span id="el<?php echo $restore_task_delete->RowCnt ?>_restore_task_username" class="control-group restore_task_username">
...
<?php
	$restore_task_delete->Recordset->MoveNext();
}
$restore_task_delete->Recordset->Close();
?>
</tbody>
...
<script type="text/javascript">
frestore_taskdelete.Init();
</script>
<?php
$restore_task_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$restore_task_delete->Page_Terminate();
?>
