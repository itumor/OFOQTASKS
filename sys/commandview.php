...
$command_view = NULL; // Initialize page object first

class ccommand_view extends ccommand {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["command_id"] <> "") {
			$this->RecKey["command_id"] = $_GET["command_id"];
			$KeyUrl .= "&command_id=" . urlencode($this->RecKey["command_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["command_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["command_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "commandlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "commandlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_command\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_command',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fcommandview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($command_view)) $command_view = new ccommand_view();

// Page init
$command_view->Page_Init();

// Page main
$command_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$command_view->Page_Render();
?>
...
// Page object
var command_view = new ew_Page("command_view");
command_view.PageID = "view"; // Page ID
var EW_PAGE_ID = command_view.PageID; // For backward compatibility

// Form object
var fcommandview = new ew_Form("fcommandview");

// Form_CustomValidate event
fcommandview.Form_CustomValidate = 
...
fcommandview.ValidateRequired = true;
<?php } else { ?>
fcommandview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fcommandview.Lists["x_task_id"] = {"LinkField":"x_task_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_task_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fcommandview.Lists["x_server_id"] = {"LinkField":"x_server_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_server_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $command_view->ExportOptions->Render("body") ?>
<?php if (!$command_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($command_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $command_view->ShowPageHeader(); ?>
<?php
$command_view->ShowMessage();
?>
<?php if ($command->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($command_view->Pager)) $command_view->Pager = new cNumericPager($command_view->StartRec, $command_view->DisplayRecs, $command_view->TotalRecs, $command_view->RecRange) ?>
<?php if ($command_view->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
<div class="pagination"><ul>
	<?php if ($command_view->Pager->FirstButton->Enabled) { ?>
	<li><a href="<?php echo $command_view->PageUrl() ?>start=<?php echo $command_view->Pager->FirstButton->Start ?>"><?php echo $Language->Phrase("PagerFirst") ?></a></li>
	<?php } ?>
	<?php if ($command_view->Pager->PrevButton->Enabled) { ?>
	<li><a href="<?php echo $command_view->PageUrl() ?>start=<?php echo $command_view->Pager->PrevButton->Start ?>"><?php echo $Language->Phrase("PagerPrevious") ?></a></li>
	<?php } ?>
	<?php foreach ($command_view->Pager->Items as $PagerItem) { ?>
		<li<?php if (!$PagerItem->Enabled) { echo " class=\" active\""; } ?>><a href="<?php if ($PagerItem->Enabled) { echo $command_view->PageUrl() . "start=" . $PagerItem->Start; } else { echo "#"; } ?>"><?php echo $PagerItem->Text ?></a></li>
	<?php } ?>
	<?php if ($command_view->Pager->NextButton->Enabled) { ?>
	<li><a href="<?php echo $command_view->PageUrl() ?>start=<?php echo $command_view->Pager->NextButton->Start ?>"><?php echo $Language->Phrase("PagerNext") ?></a></li>
	<?php } ?>
	<?php if ($command_view->Pager->LastButton->Enabled) { ?>
	<li><a href="<?php echo $command_view->PageUrl() ?>start=<?php echo $command_view->Pager->LastButton->Start ?>"><?php echo $Language->Phrase("PagerLast") ?></a></li>
	<?php } ?>
</ul></div>
</td>
</tr></tbody></table>
...
<?php } ?>
<form name="fcommandview" id="fcommandview" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="command">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_commandview" class="table table-bordered table-striped">
